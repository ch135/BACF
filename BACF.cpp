...
struct  Params {
	...
	//filter settings
	// model_sz 和 target_padding 直接决定边框的位置
	cv::Size model_sz = cv::Size(200, 200);		// 瓶颈框越小，最后搜索的区域也会变小
	float target_padding = 2.0;	// 边框越大，搜索区域会变大，但用于提取特征的区域会变小，目标追踪区域性能会降低
	//learning parameters
	float update_rate = 0.009;	//决定后面边框位置的变化
	float sigma_factor = 1.0 / 16.0;

	//scale settings
	float scale_step = 1.10;
	int num_scales = 1;
...
struct TrackedRegion {
	...
	TrackedRegion(const cv::Rect box) : center(box.x + round((float)box.size().width / 2.0),	// 获取右下角坐标
		box.y + round((float)box.size().height / 2.0)),
		size(box.size()) { }

	cv::Rect Rect() const {	// 获取左上角坐标
		return cv::Rect(center.x - floor((float)size.width / 2.0),
			center.y - floor((float)size.height / 2.0),
			size.width, size.height);
	}
	// (w* target_padding，h* target_padding)
...
// 获取检测区域，求对应区域的傅里叶信息，再将信息用拉格朗日乘算法进行优化
...
void BACF::detect(const cv::Mat& image) {
	...
	cv::Point2i translation(round(shift_index(maxpos.x, response.cols)*scale_factor),	// round() 四舍五入
...
/*
DFT：离散傅里叶变换。将图像的时域采样转化为图像的频域采样。
之后采用 FFT（快速傅里叶变换），将多块DFT 信息联合在一起
*/
...
public:
	...
	struct handlerT {
		bool isDrawing;
		Rect2d box;
		Mat image;	// 保存当前帧图像的信息，用于恢复去除图像边框信息，避免干扰
...
Rect2d BoxExtractor::extract(const std::string& windowName, Mat img, bool showCrossair) {
	...
	while (!(key == 32 || key == 27 || key == 13)) {
		...
		// draw cross air in the middle of bounding box；在边框中画十字架
		if (showCrossair) {
			// horizontal line
			line(
				params.image,
...
int main(int argc, char** argv) {
	...
	cout <<"FPS: "<< number / (end_time-start_time)<<endl;
